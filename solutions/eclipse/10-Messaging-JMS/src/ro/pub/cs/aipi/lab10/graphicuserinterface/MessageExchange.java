...
public class MessageExchange {
			...
			public void handle(ActionEvent event) {
				...
				if (interlocutorUserName.isEmpty() || messageContent.isEmpty()) {
					...
					if (conversationHistory.isEmpty()) {
						...
						conversationHistoryTextArea.setText(currentUserName + "(" + messageDateTime.get(Calendar.DAY_OF_MONTH) + "/" + (messageDateTime.get(Calendar.MONTH) + 1) + "/" + messageDateTime.get(Calendar.YEAR) + " " + messageDateTime.get(Calendar.HOUR_OF_DAY) + ":" + messageDateTime.get(Calendar.MINUTE) + ":" + messageDateTime.get(Calendar.SECOND) +  ")> " + messageContent);
						contactsList.handleCurrentMessagingWindows(interlocutorUserName, messageExchange);
					} 
					else {
						// TODO (exercise 9): include the date and time of the message
						conversationHistoryTextArea.appendText("\n" + currentUserName + "(" + messageDateTime.get(Calendar.DAY_OF_MONTH) + "/" + (messageDateTime.get(Calendar.MONTH) + 1) + "/" + messageDateTime.get(Calendar.YEAR) + " " + messageDateTime.get(Calendar.HOUR_OF_DAY) + ":" + messageDateTime.get(Calendar.MINUTE) + ":" + messageDateTime.get(Calendar.SECOND) +  ")> " + messageContent);
					}
	...
	public void handleConversationHistory(String messageContent, GregorianCalendar messageDateTime) {
		...
		// TODO (exercise 9): include the date and time of the message
		if (conversationHistory.isEmpty()) {
			conversationHistoryTextArea.setText(interlocutorUserName + "(" + messageDateTime.get(Calendar.DAY_OF_MONTH) + "/" + (messageDateTime.get(Calendar.MONTH) + 1) + "/" + messageDateTime.get(Calendar.YEAR) + " " + messageDateTime.get(Calendar.HOUR_OF_DAY) + ":" + messageDateTime.get(Calendar.MINUTE) + ":" + messageDateTime.get(Calendar.SECOND)+ ")> " + messageContent);
			contactsList.handleCurrentMessagingWindows(interlocutorUserName, messageExchange);
		} 
		else {
			conversationHistoryTextArea.appendText("\n" + interlocutorUserName + "(" + messageDateTime.get(Calendar.DAY_OF_MONTH) + "/" + (messageDateTime.get(Calendar.MONTH) + 1) + "/" + messageDateTime.get(Calendar.YEAR) + " " + messageDateTime.get(Calendar.HOUR_OF_DAY) + ":" + messageDateTime.get(Calendar.MINUTE) + ":" + messageDateTime.get(Calendar.SECOND)+ ")> " + messageContent);
		}
	}
}